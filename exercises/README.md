...
This directory holds the exercise code for the learning roadmap. The
final, currently-running k8s cluster itself lives at
[`../cluster/`](../cluster/) — see its own
[`README.md`](../README.md) for the cluster-focused overview. This file
covers the full progression, exercise by exercise.
...
script ([`scripts/run-ci-job.sh`](01-local-container-ci/scripts/run-ci-job.sh)):
...
BuildKit security comparison](01-local-container-ci/dind-experiment/README.md).
...
[queue-driven runner controller](02-kubernetes-ci/scripts/runner-controller.sh),
a default-deny egress policy, and Prometheus/Grafana CI metrics
([`monitoring-values.yaml`](02-kubernetes-ci/monitoring-values.yaml)).
...
[startup-time script](04-kata-microvms/benchmarks/measure-startup.sh).
...
- **But the CI runner could freely reach the internal container
  ...
  ([`arc-runner-egress-policy.yaml`](06-github-actions-arc/security-drill/arc-runner-egress-policy.yaml)).
